...
`domain/` imports nothing but `@anomalia/contracts`. That is what makes the
...
The consumer imports an *interface* from `@anomalia/contracts`, never the
...
3. Postgres **row-level security** — even a repository bug cannot read another
   tenant's rows. This is why the app connects as `anomalia_app` and not as the
