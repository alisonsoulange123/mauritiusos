...
describe('isSameOrigin', () => {
  it('accepts a matching origin, port included', () => {
    ...
    expect(isSameOrigin('https://app.anomalia.io', 'app.anomalia.io')).toBe(true);
  });

  it('rejects another origin', () => {
    expect(isSameOrigin('https://evil.example', 'app.anomalia.io')).toBe(false);
    ...
    expect(isSameOrigin('https://anomalia.io.evil.example', 'anomalia.io')).toBe(false);
  });
  ...
  it('rejects a missing or unparseable origin', () => {
    ...
    expect(isSameOrigin(null, 'app.anomalia.io')).toBe(false);
    expect(isSameOrigin('null', 'app.anomalia.io')).toBe(false);
    expect(isSameOrigin('https://app.anomalia.io', null)).toBe(false);
  });
});
