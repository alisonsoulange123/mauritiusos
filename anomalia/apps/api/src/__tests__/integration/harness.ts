...
const DATABASE_URL = process.env.TEST_DATABASE_URL ?? 'postgres://anomalia:anomalia@localhost:5432/anomalia';
  ...
  const request = async (
    ...
    return fetch(`${url}${path}`, {
      ...
      headers: {
        ...
        'x-anomalia-tenant': tenantOverride ?? tenant,
  ...
  if (!existsSync(compiled)) {
    throw new Error(
      'dist/app.module.js is missing — integration tests boot the compiled app.\n' +
        'Run `pnpm --filter @anomalia/api build` first, or use `pnpm test:integration`.',
    );
  }
