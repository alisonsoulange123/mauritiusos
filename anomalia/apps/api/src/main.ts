...
async function bootstrap(): Promise<void> {
  ...
  app.enableCors({
    ...
    allowedHeaders: ['content-type', 'authorization', 'x-anomalia-tenant', 'x-trace-id'],
  ...
  if (bootConfig.NODE_ENV !== 'production') {
    const document = SwaggerModule.createDocument(
      app,
      new DocumentBuilder()
        .setTitle('ANOMALIA Platform API')
        .setDescription(
