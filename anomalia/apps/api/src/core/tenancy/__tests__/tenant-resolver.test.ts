...
describe('slugFromHost', () => {
  it('reads the tenant from a subdomain', () => {
    expect(slug('portal.mauritius.anomalia.io')).toBe('mauritius');
    expect(slug('mauritius.anomalia.io')).toBe('mauritius');
  });

  it('ignores www', () => {
    expect(slug('www.anomalia.io')).toBeUndefined();
  });
  ...
  it('falls through for a bare host', () => {
    ...
    expect(slug('anomalia.io')).toBeUndefined();
  });
  ...
  it('strips a port before counting labels', () => {
    expect(slug('portal.mauritius.anomalia.io:443')).toBe('mauritius');
