...
import { isEventName, type ModuleKey } from '@reef-technologies/contracts';
import { flagNameFor, isModuleEnabled, type FlagSet } from '@reef-technologies/config';
import type { ReefTechnologiesModuleDefinition, ModuleManifest, ModuleManifestEntry } from './module.definition.js';
...
export interface LoadModulesInput {
  registry: readonly ReefTechnologiesModuleDefinition[];
  flags: FlagSet;
  nodeEnv: string;
}
...
export interface LoadModulesResult {
  ...
  configSchemas: Array<{ key: string; schema: NonNullable<ReefTechnologiesModuleDefinition['configSchema']> }>;
}
...
export function loadModules({ registry, flags, nodeEnv }: LoadModulesInput): LoadModulesResult {
  ...
  const orderedDefinitions = order
    ...
    .filter((definition): definition is ReefTechnologiesModuleDefinition => definition !== undefined);
...
function indexRegistry(
  registry: readonly ReefTechnologiesModuleDefinition[],
  problems: string[],
): Map<ModuleKey, ReefTechnologiesModuleDefinition> {
  const byKey = new Map<ModuleKey, ReefTechnologiesModuleDefinition>();
...
function validateEventNames(
  ...
  for (const definition of registry) {
    for (const name of [...(definition.publishes ?? []), ...(definition.subscribes ?? [])]) {
      if (!isEventName(name)) {
        problems.push(
          `module "${definition.key}" references unknown event "${name}" — add it to the catalog in @reef-technologies/contracts`,
        );
      }
    }
  }
...
interface ResolveDisabledInput {
  registry: readonly ReefTechnologiesModuleDefinition[];
  byKey: Map<ModuleKey, ReefTechnologiesModuleDefinition>;
  flags: FlagSet;
  nodeEnv: string;
  problems: string[];
}
...
function validateContracts(
  enabled: readonly ReefTechnologiesModuleDefinition[],
  problems: string[],
): void {
